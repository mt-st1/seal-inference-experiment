...
vector<int> generate_log2_modulus(size_t level, size_t log2_q0, size_t log2_qi, size_t log2_ql) {
...
int main(int argc, char* argv[]) {
  ...
  parser.add<size_t>("level", 'L', "Initial level of ciphertext (Multiplicative depth)");
  parser.add<size_t>("q0", 0, "Bit number of the first prime in coeff_modulus");
  parser.add<size_t>("qi", 0, "Bit number of intermediate primes in coeff_modulus");
  ...
  vector<int> prime_bit_sizes = generate_log2_modulus(level, log2_q0, log2_qi, log2_ql);

  seal::EncryptionParameters params(seal::scheme_type::ckks);
  params.set_poly_modulus_degree(poly_modulus_degree);
  params.set_coeff_modulus(seal::CoeffModulus::Create(poly_modulus_degree, prime_bit_sizes));
  ...
  auto secrets_ofs = [&](const string &fname_suffix) -> const unique_ptr<ofstream>& {
    ofs_ptr.reset(new ofstream(constants::fname::SECRETS_DIR + "/" + fname_prefix + fname_suffix, ios::binary));
    return ofs_ptr;
  };

  if (!fs::exists(constants::fname::SECRETS_DIR)) fs::create_directory(constants::fname::SECRETS_DIR);
